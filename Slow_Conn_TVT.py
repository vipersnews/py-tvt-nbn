...
def make_connection (ip, username, password):
		return netmiko.ConnectHandler(device_type='cisco_ios', ip=ip, username=username, password=password, fast_cli=False)
...
for ip in ips:
	...
	try:
		...
		for commands in commands_list:
			output = net_connect.send_command_timing(commands, delay_factor=4)
			results = output + '\n'
